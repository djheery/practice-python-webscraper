...
initial_query = '/contacts/listing/search?query=Actors'
...
def check_pagination(page):
  r = s.get(page)
  soup = BeautifulSoup(r.content, 'html.parser')
  paginationContainer = soup.find('div', {'class':'paginationContainer'})
  if paginationContainer != None: 
    array = []
    links = paginationContainer.find_all('a')
    for link in links:
...
links_to_search = check_pagination(f'{base_url}{initial_query}')
data = []
for link in links_to_search:
  soup = get_data(f'{base_url}{link}')
  links = get_links(soup)
  get_scraping(links, data)
